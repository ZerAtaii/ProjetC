...
double getTotalEarnings(bankAccount acc) {
	double totalEarnings = 0.0;
	int i;
	for (i=0; i<6; i++) {
		totalEarnings = totalEarnings + getTotalEarningType(acc)[i];
	}
	return totalEarnings;
}

double *getStatsEarnings(bankAccount acc) {
	double *percentagesEarnings = malloc(6*sizeof(double));
	int i;
	for (i=0; i<6; i++) {
		percentagesEarnings[i] = 100*(getTotalEarningType(acc)[i]/getTotalEarnings(acc));
	}
	return percentagesEarnings;
	
}
void updateAmount(bankAccount acc) {
	int i;
	for (i=0; i<acc.size; i++) {
		acc.amount = acc.amount + (acc.motions[i]).amountSpent;
	}
}

void askBudget(float *tab) {
	int i;
	for (i=0; i<7; i++) {
		printf("What is your budget in ");
		printSpending(i);
		printf("?");
		float a;
		scanf("%f",&a);
